CREATE OR REPLACE TABLE `aumc_uo_and_aki.c_hourly_uo_95all_temp` AS
...
WITH
  ...
  RAW_ARRAY_OF_TIMES AS (
    ...
    FROM
      `aumc_uo_and_aki.b_uo_rate_temp` uo
    GROUP BY
      STAY_ID
  ),
...
FROM
  ...
  LEFT JOIN `original.admissions` c ON c.admissionid = a.STAY_ID
GROUP BY
  a.STAY_ID,
  a.T_PLUS,
  a.TIME_INTERVAL_STARTS,
