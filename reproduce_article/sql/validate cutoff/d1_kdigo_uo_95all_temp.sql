CREATE OR REPLACE TABLE
  `aumc_uo_and_aki.d1_kdigo_uo_95all_temp` AS
WITH
  uo_6hr AS (
    SELECT
      index_hour.stay_id,
    ...
    FROM
      `aumc_uo_and_aki.c_hourly_uo_95all_temp` index_hour
      -- this join gives all UO measurements over the 6 hours preceding this row
      LEFT JOIN `aumc_uo_and_aki.c_hourly_uo_95all_temp` uo ON index_hour.stay_id = uo.stay_id
...
FROM
  `aumc_uo_and_aki.c_hourly_uo_95all_temp` AS index_hour
